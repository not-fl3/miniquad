...
pub unsafe fn sapp_run(desc: *const sapp_desc) -> ::std::os::raw::c_int {
    ...
    SAPP_DESC = Some(*desc);
    SAPP_DESC.unwrap_or_else(|| panic!()).init_userdata_cb.unwrap_or_else(|| panic!())(USER_DATA);
...
pub fn console_log(msg: &str) {
    ...
    unsafe { test_log(string.as_ptr()); }
}
#[no_mangle]
pub extern "C" fn frame() {
    unsafe { SAPP_DESC.unwrap_or_else(|| panic!())
        .frame_userdata_cb.unwrap_or_else(|| panic!())(USER_DATA); }
}
...
pub extern "C" fn mouse_move(x: i32, y: i32) {
    ...
    unsafe { SAPP_DESC.unwrap_or_else(|| panic!())
        .event_userdata_cb.unwrap_or_else(|| panic!())(&event as *const _, USER_DATA); }
}

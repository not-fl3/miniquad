use super::libwayland_client::{wl_buffer, wl_proxy, wl_shm, wl_shm_format_WL_SHM_FORMAT_ARGB8888, wl_shm_pool, LibWaylandClient, WL_SHM_CREATE_POOL, WL_SHM_POOL_CREATE_BUFFER, WL_SHM_POOL_DESTROY};
...
pub unsafe fn create_shm_buffer(libwayland: &mut LibWaylandClient, shm: *mut wl_shm, width: i32, height: i32, pixels: &[u8]) -> *mut wl_buffer {
	let stride = width * 4;
	...
	let data = libc::mmap(std::ptr::null_mut(), length as _, libc::PROT_READ | libc::PROT_WRITE, libc::MAP_SHARED, fd, 0);
	...
	let pool = wl_request_constructor!(libwayland, shm, WL_SHM_CREATE_POOL, libwayland.wl_shm_pool_interface, fd, length);
	libc::close(fd);

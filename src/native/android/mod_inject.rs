// Imagine crate A depending on miniquad_wasm_bindgen.
// On android A is being compiled to .so and this .so is being loaded
// by "System.loadLibrary("")" java call.
// extern "C" functions from miniquad_wasm_bindgen are right there, in the .so. But somehow
...
//
// But we really need to be able to make miniquad_wasm_bindgen's functions visible to java.
//
...
// and they just forward the call to the real implementation inside miniquad_wasm_bindgen
// Note that because it is being injected - we might not have neither miniquad_wasm_bindgen

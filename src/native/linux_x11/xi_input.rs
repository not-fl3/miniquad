...
type XQueryExtension = fn(_: *mut Display, _: *const libc::c_char, _: *mut libc::c_int, _: *mut libc::c_int, _: *mut libc::c_int) -> libc::c_int;
type XIQueryVersion = fn(dpy: *mut Display, major_version_inout: *mut libc::c_int, minor_version_inout: *mut libc::c_int) -> libc::c_int;
type XISelectEvents = fn(dpy: *mut Display, win: Window, masks: *mut XIEventMask, num_masks: libc::c_int);
...
impl LibXi {
	...
	pub unsafe fn query_xi_extension(&mut self, libx11: &mut libx11::LibX11, display: *mut Display) {
		let mut ev = 0;
		let mut err = 0;
		let mut xi_opcode = 0;

		if (libx11.XQueryExtension)(display, b"XInputExtension\x00" as *const u8 as *const libc::c_char, &mut xi_opcode, &mut ev, &mut err) == 0 {
			return;
		}
		...
		(self.XISelectEvents)(
			...
			(*(*(display as _XPrivDisplay)).screens.offset((*(display as _XPrivDisplay)).default_screen as isize)).root,
			&mut masks,
			1 as libc::c_int,
		);
	...
	pub unsafe fn read_cookie(&mut self, xcookie: &mut libx11::XGenericEventCookie, display: *mut Display) -> (f64, f64) {

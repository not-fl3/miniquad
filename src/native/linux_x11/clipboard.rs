...
unsafe fn get_clipboard(libx11: &mut LibX11, display: *mut Display, window: Window, bufname: *const libc::c_char, fmtname: *const libc::c_char) -> Option<String> {
	...
	let propid = (libx11.XInternAtom)(display, b"XSEL_DATA\x00" as *const u8 as *const libc::c_char, false as _);
	let incrid = (libx11.XInternAtom)(display, b"INCR\x00" as *const u8 as *const libc::c_char, false as _);
...
unsafe fn claim_clipboard_ownership(libx11: &mut LibX11, display: *mut Display, window: Window, bufname: *const libc::c_char, message: String) {
	let selection = (libx11.XInternAtom)(display, bufname as *const u8 as *const libc::c_char, 0 as libc::c_int);
...
pub(crate) unsafe fn respond_to_clipboard_request(libx11: &mut LibX11, display: *mut Display, event: *const XEvent) {
	...
	let UTF8 = (libx11.XInternAtom)(display, b"UTF8_STRING\x00" as *const u8 as *const libc::c_char, 1 as libc::c_int);
	...
	if xselectionrequest.target == UTF8 {
		...
		(libx11.XSendEvent)(display, ev.requestor, 0 as libc::c_int, 0 as libc::c_int as libc::c_long, &mut ev as *mut XSelectionEvent as *mut XEvent);
	} else {
...
impl X11Clipboard {
	pub fn new(libx11: LibX11, display: *mut Display, window: Window) -> X11Clipboard {
		X11Clipboard { libx11, display, window }
	}
}
impl crate::native::Clipboard for X11Clipboard {
	fn get(&mut self) -> Option<String> {
		...
		unsafe { get_clipboard(&mut self.libx11, self.display, self.window, bufname.as_ptr(), fmtname.as_ptr()) }
	}
	...
	fn set(&mut self, data: &str) {
		...
		unsafe {
			claim_clipboard_ownership(&mut self.libx11, self.display, self.window, bufname.as_ptr(), data.to_owned());
		};
	}
}

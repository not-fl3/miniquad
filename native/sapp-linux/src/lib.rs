...
pub const _SAPP_EVENTTYPE_FORCE_U32: sapp_event_type = 2147483647;
pub const _SAPP_EVENTTYPE_NUM: sapp_event_type = 21;
pub const SAPP_EVENTTYPE_QUIT_REQUESTED: sapp_event_type = 20;
...
pub unsafe extern "C" fn _sapp_x11_query_system_dpi() {
    ...
    if !rms.is_null() {
        ...
        if !db.is_null() {
            ...
            let mut type_0 = std::ptr::null_mut();
            if XrmGetResource(
                db,
            ...
            {
                if !type_0.is_null()
                    && strcmp(type_0, b"String\x00" as *const u8 as *const libc::c_char)
                        == 0 as libc::c_int
                {
...
pub unsafe extern "C" fn _sapp_x11_process_event(mut event: *mut XEvent) {
    match (*event).type_0 {
        ...
        3 => {
            ...
            if key_0 as libc::c_uint != SAPP_KEYCODE_INVALID as libc::c_int as libc::c_uint {
                let mods_0 = _sapp_x11_mod((*event).xkey.state as libc::c_int);
                _sapp_x11_key_event(SAPP_EVENTTYPE_KEY_UP, key_0, false, mods_0);
            }
        }
        4 => {
...
pub unsafe extern "C" fn _sapp_init_event(mut type_0: sapp_event_type) {
    memset(
...
pub unsafe extern "C" fn _sapp_x11_app_event(mut type_0: sapp_event_type) {
    if _sapp_events_enabled() {
        _sapp_init_event(type_0);
        _sapp_call_event(&mut _sapp.event);
    };
